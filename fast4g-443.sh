...
EOF
  cat >key.key <<EOF
#bạn hãy copy dòng key ssl vào đây
EOF
  cat >crt.crt <<EOF
#bạn hãy copy cert ssl vào đây
EOF
  cat >config.yml <<EOF
Log:
  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertFile: /etc/cloud/ssl/cert-net/fast4g.cert 
        KeyFile: /etc/cloud/ssl/cert-net/fast4g.key
        Provider: alidns 
        Email: test@me.com
        DNSEnv: 
